...
import { ETHIOPIAN_FOODS, getFoodStatus } from '@/lib/foods';
...
export default function App() {
  ...
  // Session tracking to ensure APIs are linked
  ...
  // Create a tracking session on the backend when moving to menu or insight
    ...
    try {
      const response = await fetch('/api/session', {
        ...
        body: JSON.stringify({
          ...
          hypertension: conditions.hbp,
          pregnant: conditions.pregnant,
  ...
  const handleGetRecommendation = async () => {
    ...
    try {
      ...
      if (!currentSessionId) {
        throw new Error('Failed to establish session for recommendation');
      }

      const response = await fetch(`/api/recommendation?session_id=${currentSessionId}`);
      ...
      <header className="hidden md:flex fixed top-0 w-full z-50 bg-[#00694c] text-white border-b border-[#bccac1]/20">
        <div className="flex w-full items-center justify-between max-w-7xl mx-auto h-16 px-8">
          ...
          <nav className="flex items-center gap-4">
            <button 
              onClick={() => { setPage('profile'); setRecommendation(null); setSessionId(null); }}
          ...
          {page === 'insight' && (
            ...
            >
              ...
              {loading ? (
                ...
                /* Dynamic Gemini/Groq returned content template */
                <div className="flex flex-col gap-6 text-sm" id="content-state">
                  ...
                  <section className="bg-white border border-[#bccac1] rounded-2xl p-6 relative overflow-hidden shadow-sm">
                    ...
                    <div className="absolute inset-0 opacity-5 pointer-events-none" style={{ backgroundImage: 'radial-gradient(circle at 2px 2px, #00694c 1.5px, transparent 0)', backgroundSize: '16px 16px' }} />
